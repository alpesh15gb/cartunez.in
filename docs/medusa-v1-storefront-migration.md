...
| `src/lib/data/variants.ts` | v2 `fields=*images` | `GET /store/variants/:id?expand=images,product`; `{variant}` | Native numeric `calculated_price`; medium (currently unused by pages) |
...
| `src/lib/data/payment.ts`, checkout payment UI | v2 provider/session IDs and `payment_collection` | Providers from `GET /store/regions/:id` (`region.payment_providers`); create/select v1 sessions on cart | v1 has no `/store/payment-providers` route; manual provider ID supported; critical |
