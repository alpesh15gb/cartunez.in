...
export async function searchVehicles(params: {
  ...
  return apiFetch(`/api/v1/vehicles/search?${q}`);
}
...
export async function fetchBlogPosts(params?: { limit?: number; offset?: number }): Promise<BlogPost[]> {
  ...
  return apiFetch(`/api/v1/blogs/posts?${q}`);
}

export async function fetchBlogPost(slug: string): Promise<BlogPost> {
  return apiFetch(`/api/v1/blogs/posts/${slug}`);
}
...
export async function fetchReviews(productId: string): Promise<Review[]> {
  return apiFetch(`/api/v1/reviews?product_id=${encodeURIComponent(productId)}`);
}
...
export async function createReview(data: {
  ...
  return apiFetch('/api/v1/reviews', { method: 'POST', body: JSON.stringify(data) });
}
...
export async function createSupportTicket(data: {
  ...
  return apiFetch('/api/v1/support/tickets', { method: 'POST', body: JSON.stringify(data) });
}
...
export async function bookInstallation(data: {
  ...
  return apiFetch('/api/v1/installation/bookings', { method: 'POST', body: JSON.stringify(data) });
}
...
export async function submitBulkEnquiry(data: {
  ...
  return apiFetch('/api/v1/bulk-enquiry/enquiries', { method: 'POST', body: JSON.stringify(data) });
}
...
export async function submitDealerEnquiry(data: {
  ...
  return apiFetch('/api/v1/dealers/enquiries', { method: 'POST', body: JSON.stringify(data) });
}
...
export async function fetchGallery(params?: { limit?: number; category?: string }): Promise<GalleryItem[]> {
  ...
  return apiFetch(`/api/v1/gallery?${q}`);
}
...
export async function fetchInstagramReels(): Promise<InstagramReel[]> {
  const data = await apiFetch<{ reels: InstagramReel[]; count: number }>(
    '/api/v1/social/instagram/reels'
  );
  return data.reels || [];
}
