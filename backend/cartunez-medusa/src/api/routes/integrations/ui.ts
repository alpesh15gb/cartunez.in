...
.view-btn {
  padding: 6px 16px; border-radius: var(--radius); background: var(--bg-input);
  border: 1px solid var(--border); color: var(--text-secondary); cursor: pointer; font-size: 13px;
}
.view-btn.active { background: var(--accent); color: white; border-color: var(--accent); }
.secret-masked { font-family: monospace; color: var(--text-secondary); font-size: 12px; }
`;
...
async function testConnection() {
  ...
  // Generate a temp ID or just validate we can create first
  // Test button first creates a draft connection if needed
  try {
    // For test, try creating first then deleting
    saveConnection(true).then(async (connId) => {
      if (connId) {
        const data = await api(\`/admin/integrations/\${connId}/test\`);
        result.innerHTML = data.success
          ? '<div class="alert success">✓ Connection successful (status ' + data.status + ')</div>'
